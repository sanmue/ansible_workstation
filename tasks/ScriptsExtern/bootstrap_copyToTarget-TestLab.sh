...
# current:
rsync -avPEzh --stats --exclude={"bootstrap_copyToTarget-TestLab.sh","config_workstation-addrepos-ubuntu.sh","config_workstation-desktopPreferences-terminal.sh","config_all-servcices-misc-vim.sh","*.yml*"} --include="*.sh" "./" "${userid}@${targetIP}:~"
