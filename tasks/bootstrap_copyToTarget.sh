...
rsync -avPEzh --stats --exclude={"bootstrap_copyToTarget.sh","config_workstation-desktopPreferences-terminal.sh","config_workstation-basic-vim.sh","*.yml*"} --include="*.sh" "./" "${userid}@${targetIP}:~"
